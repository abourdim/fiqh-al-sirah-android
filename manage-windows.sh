#!/usr/bin/env bash
# ============================================================================
#  Fiqh al-Sirah Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/23-fiqh-al-sirah}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.fiqhalsirah}"
AVD_NAME="${AVD_NAME:-fiqh_al_sirah_test}"
...
cmd_test() {
  ...
  local out="/tmp/fiqh-al-sirah-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Fiqh al-Sirah Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
